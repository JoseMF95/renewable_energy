...
Let's upload our data for public investment
```{r}
df_finance <- read_csv("./raw_data/irena_finance.csv")
```
we want to analyze public investment from 1990 to the lastest data we have which is 2021. Here are some preprocessing steps that we did. 
```{r}
